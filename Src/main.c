...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
...
#ifdef DEBUG
	len = sprintf(BUFF, "%u\t%u\t%u\n\0", NEC1.i, NEC1.gpio, NEC1.count);
	CDC_Transmit_FS(BUFF, len);
#endif

	if (NEC1.gpio && NEC1.count > 890 && NEC1.count < 950) {
		NEC1.init_seq = 1;
		NEC1.complet = 0;
	}

	if (!NEC1.gpio && NEC1.count > 435 && NEC1.count < 450 && NEC1.init_seq) {
		NEC1.i = -1;
		NEC1.repeat = 0;
		NEC1.init_seq = 0;
	}

	if (!NEC1.gpio && NEC1.count > 220 && NEC1.count < 230 && NEC1.init_seq) {
		NEC1.i = -1;
		NEC1.init_seq = 0;
		NEC1.repeat++;
		NEC1.complet = 1;
	...
	if (NEC1.gpio && NEC1.count > 50 && NEC1.count < 70) {
		NEC1.i++;
	}

	if (!NEC1.gpio && NEC1.count > 50 && NEC1.count < 180) {
		switch (NEC1.i / 8) {
		case 0:
			if (NEC1.count > 100) {
